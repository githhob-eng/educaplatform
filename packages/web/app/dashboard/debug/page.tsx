  ...
  const syncToBackend = async () => {
    try {
      for (const usr of hierarchyData) {
        const existing = await fetch(`https://educaplatform-bg8vrm5zl-premedics-projects.vercel.app/api/users`);
        const existingUsers = await existing.json();
        const userExists = existingUsers.find((u: any) => u.id === usr.id);

        if (!userExists) {
          const response = await fetch('https://educaplatform-bg8vrm5zl-premedics-projects.vercel.app/api/users', {
            method: 'POST',
